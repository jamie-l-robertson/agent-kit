...
description: >-
  ...
  layout and breakpoints; CSS/SCSS modules or Tailwind; hover/focus
  styling and motion; baseline accessible markup. Use when a request names
  a visual element (hero, nav, card, grid, spacing, breakpoint,
  animation). Not for deep WCAG audits (accessibility), server/CMS/API
  (backend), harness/CI tests-only (tester), or docs (documenter).
---
