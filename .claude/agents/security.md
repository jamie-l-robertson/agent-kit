...
description: >-
  ...
  in code, injection/XSS/CSRF, dependency/CVE hygiene, and surgical
  security remediations. Use for security audits, vulnerability fixes, or
  auth boundary work. Not for PII/compliance (risk), cloud secret
  stores/DNS-as-code (infrastructure), CI (devops), or incidental PR
  smells (reviewer → Recommend next: security). Default audit-only unless
  asked to fix.
---
