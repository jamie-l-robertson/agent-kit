...
- **Skills**: kit — `a11y-wcag`, `agent-memory`, `architecture-review`, `brief-hygiene`, `browser-test`, `code-review`, `issue-intake`, `manager`, `perf-audit`, `response-sanity`, `setup`, `sync-project-skills`, `verify-evidence`; project — `bro`, `grill-with-docs`, `simplify`, `to-issues`. Inventory: `.claude/memory/skills-inventory.md`.
