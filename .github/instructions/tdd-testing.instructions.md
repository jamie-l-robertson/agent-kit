...
For Playwright e2e/a11y when the project uses them: attempt the run first (`webServer` can start the dev server; allow ~180s cold start). Reserve blocked for failed boot, auth, or missing required env secrets from `AGENTS.md` — do not silently skip.
